...
<button
                    ...
                    style={{
                      ...
                      borderBottom: isRegActive ? "3px solid #1a365d" : "3px solid transparent",
...
<div style={{ marginBottom: 10, fontSize: "0.8rem", fontWeight: "bold", color: "#4a5568" }}>
                    ...
                    {selectedRegion === "すべて" ? "都道府県" : `${selectedRegion}の都道府県`}
</div>

                  {regionalPrefectures

                    .filter((regionGroup) => selectedRegion === "すべて" || regionGroup.region === selectedRegion)
...
<span style={{ color: "#1a365d", marginRight: 8 }}>|</span>
            ...
            {viewMode === "all" ? `${selectedRegion === "すべて" ? "" : selectedRegion + "の"}案件一覧` : viewMode === "applied" ? "応募済み案件" : viewMode === "favorites" ? "お気に入り案件" : "閲覧履歴"} ({filteredProjects.length}件)
</h2>
...
<div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: 25 }}>
              ...
              {totalPages > 1 && (
<div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 6, marginTop: 40, marginBottom: 40, flexWrap: "wrap" }}>
<button onClick={() => changePage(1)} disabled={currentPage === 1} style={{ ...styles.pageBtn, opacity: currentPage === 1 ? 0.4 : 1, cursor: currentPage === 1 ? "not-allowed" : "pointer" }}>
&lt;&lt;
</button>
<button onClick={() => changePage(currentPage - 1)} disabled={currentPage === 1} style={{ ...styles.pageBtn, opacity: currentPage === 1 ? 0.4 : 1, cursor: currentPage === 1 ? "not-allowed" : "pointer" }}>
&lt;
</button>
 
                  {currentPage > 3 && totalPages > 6 && (
<>
<button onClick={() => changePage(1)} style={styles.pageBtn}>1</button>
<span style={{ padding: "0 5px", color: "#718096", fontWeight: "bold" }}>...</span>
</>
...
<>
<span style={{ padding: "0 5px", color: "#718096", fontWeight: "bold" }}>...</span>
<button onClick={() => changePage(totalPages)} style={styles.pageBtn}>{totalPages}</button>
</>

                  )}
 
                  <button onClick={() => changePage(currentPage + 1)} disabled={currentPage === totalPages} style={{ ...styles.pageBtn, opacity: currentPage === totalPages ? 0.4 : 1, cursor: currentPage === totalPages ? "not-allowed" : "pointer" }}>
&gt;
</button>
<button onClick={() => changePage(totalPages)} disabled={currentPage === totalPages} style={{ ...styles.pageBtn, opacity: currentPage === totalPages ? 0.4 : 1, cursor: currentPage === totalPages ? "not-allowed" : "pointer" }}>
&gt;&gt;
</button>
</div>
